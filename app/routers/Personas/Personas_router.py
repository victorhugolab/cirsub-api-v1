...
@router.post("/ou",summary="Obtiene una persona ingresando su Id.", response_model=None)
async def personas_ou(request: Request, body: PersonasOUInModel = Body(...)):
    return await personas_controller_ou(request, body)


@router.post("/in", summary="Inserta un registro en Personas.", response_model=None)
